...
func (t *PorchSuite) TestCreatingPackageRevisionWithPipelineFailure() {
	const (
	...
	err = kptfile.AddMutationFunction(fn)
	if err != nil {
		t.Fatalf("Failed to add function to Kptfile: %v", err)
	}

	err = kptfile.WriteToPackage(prr.Spec.Resources)
	if err != nil {
		t.Fatalf("Failed to write Kptfile to package resources: %v", err)
	}
	...
	pr.Spec.Lifecycle = porchapi.PackageRevisionLifecycleProposed
	gotErr := t.Client.Update(t.GetContext(), pr)
	if gotErr == nil {
